	...
	if(!(lineIntersects [eyePos player, eyePos _x, player, _x]) && {!isNil {_x getVariable "realname"}}) then {
		...
		if(count _sPos > 1 && {_distance < 10}) then {
			_text = switch (true) do {
				case ((goggles _x) in EVGA_goggles_Array): {format["<t color='#000000'>Maskierte Person</t>"];};
				case ((headgear _x) in EVGA_headgear_Array): {format["<t color='#000000'>Maskierte Person</t>"];};
				case ((uniform _x) in EVGA_uniform_Array): {format["<t color='#000000'>Maskierte Person</t>"];};
				...
				case ((!isNil {_x getVariable "name"} && playerSide == east)): {format["<t color='#0066FF'><img image='\a3\ui_f\data\gui\cfg\Ranks\major_gs.paa' size='1.5'></img></t> %1",_x getVariable ["name","Unknown Player"]]};
				default {
