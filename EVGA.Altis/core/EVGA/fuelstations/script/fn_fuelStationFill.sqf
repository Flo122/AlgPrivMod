...
if(_isTrailer == 1) then {
	_fillTrailer = ((_veh getVariable["fueltruck_fuel",0]) + _succAmount) min ([(typeOf _veh) + "_TRAILER"] call EVGA_fnc_vehFuelCap);
	_veh setVariable["fueltruck_fuel",_fillTrailer,true]; 
} else {
	_fillValue = (fuel _veh) + (_succAmount / ([typeOf _veh] call EVGA_fnc_vehFuelCap));
	if(local _veh) then {
