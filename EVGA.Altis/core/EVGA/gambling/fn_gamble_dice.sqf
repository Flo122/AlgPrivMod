...
{ 
    ...
    {
        _reroll = random 10;
        if(_reroll < EVGA_Gamble_reroll_if_won_factor_dice) then
        {
            _idx1 = floor(random count _icons);
            _idx2 = floor(random count _icons);
        };
    ...
    _image1 ctrlSetStructuredText parseText format["<img size='2.5' image='core\EVGA\icons\%1'/>", _icons select _idx1 ];
    _image2 ctrlSetStructuredText parseText format["<img size='2.5' image='core\EVGA\icons\%1'/>", _icons select _idx2 ];
...
{
    //won
    _gamble_win_factor = EVGA_Gamble_dice_win_factor_two_of_a_kind select EVGA_Gamble_casino_idx;
    if( _dice_value == 7 ) then
    {
        _gamble_win_factor = EVGA_Gamble_dice_win_factor_sum_seven select EVGA_Gamble_casino_idx;
    };
    _return = 1;
}
else
...
['Dice', _return, _gamble_val, _gamble_win_factor, _jackpot_won] call EVGA_fnc_gamble;
