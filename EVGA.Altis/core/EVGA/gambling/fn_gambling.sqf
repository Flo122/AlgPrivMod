...
if(EVGA_Gamble_get_data_thread_is_running == 0) then
{
    EVGA_Gamble_get_data_thread_is_running = 1;
    ...
    [] spawn
    {
        ...
        while {EVGA_Gamble_get_data_thread_is_running == 1} do
        {
            //hint format["EVGA_Gamble_unblock_time=%1 _time_to_unblock=%2 EVGA_Gamble_player_is_blocked=%3", EVGA_Gamble_unblock_time,_time_to_unblock,EVGA_Gamble_player_is_blocked];
            ...
            if(isNull _disp) exitWith
            {
                ...
                if(EVGA_Gamble_player_is_blocked == 0) then
                {
            ...
            if(EVGA_Gamble_player_is_blocked == 1) then
            {
                //calculate the seconds the player is blocked
                _time_to_unblock = EVGA_Gamble_unblock_time - time;
                ...
                if(_time_to_unblock < 0) then
                {
                    ...
                    EVGA_Gamble_player_is_blocked = 0;
                };
                ...
                if(_time_to_unblock > 0) then
                {
                    _gambling_blockerstatus ctrlSetStructuredText parseText format["%1 %2:%3min", EVGA_Gamble_blocked_please_wait, _minutes_to_unblock_string, _seconds_to_unblock_string];
                }
                else
                {
...
{
    EVGA_Gamble_casino_idx = _npc_paras;
    EVGA_Gamble_npc_name   = _this select 0; //save the calling NPC Name    
    //hint format["EVGA_Gamble_npc_name=%1", EVGA_Gamble_npc_name];
};

//if a gamble is active we don´t want to switch to another
if(EVGA_Gamble_roll_time_to_wait > 0) then
{
    _gamble_type = EVGA_Gamble_last_type;
}
else
{
...
//hint format["EVGA_Gamble_casino_is_illegal=%1, EVGA_Gamble_casino_idx=%2", EVGA_Gamble_casino_is_illegal select EVGA_Gamble_casino_idx, EVGA_Gamble_casino_idx];
...
_dice_rules1 ctrlSetStructuredText parseText format["Summe 7 = x%1", EVGA_Gamble_dice_win_factor_sum_seven select EVGA_Gamble_casino_idx];
_dice_rules2 ctrlSetStructuredText parseText format["6er Pasch = x%1", EVGA_Gamble_dice_win_factor_two_of_a_kind select EVGA_Gamble_casino_idx];

_bandit_rules1 ctrlSetStructuredText parseText format["zwei Gleiche = x%1", EVGA_Gamble_bandit_win_factor_two_of_a_kind select EVGA_Gamble_casino_idx];
_bandit_rules2 ctrlSetStructuredText parseText format["drei Gleiche = x%1", EVGA_Gamble_bandit_win_factor_three_of_a_kind select EVGA_Gamble_casino_idx];

_jackpot_rules ctrlSetStructuredText parseText format["%1", EVGA_Gamble_jackpot_rules_text];



if(EVGA_Gamble_casino_is_illegal select EVGA_Gamble_casino_idx == 0) then
{
...
ctrlSetText[5004, EVGA_Gamble_title_text + " [" + _legal_illegal_text + "]"];
...
EVGA_Gamble_last_type = _gamble_type;
